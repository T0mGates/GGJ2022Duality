...
export var min_x_push = 5.0
export var max_x_push = 8.0
export var min_y_push = 5.0
export var max_y_push = 22.0
...
func _ready():
	connect("scene_load", $Player, "scene_loaded")
...
func _process(delta):
	...
	#my own 'custom' ways for scaling velocity based on how close object is to vacuum... lmk if u think of something better
			if(obj.get_position().x > get_parent().get_position().x):
				...
				forces.x += lerp(min_x_push, max_x_push, get_parent().get_position().x/obj.get_position().x)
			else:
				#velocity.x += (obj.get_position().x/get_parent().get_position().x)
				forces.x -= lerp(min_x_push, max_x_push, get_parent().get_position().x/obj.get_position().x)
			if(obj.get_position().y > get_parent().get_position().y):
				forces.y += lerp(min_y_push, max_y_push, get_parent().get_position().y/obj.get_position().y)
			else:
				forces.y -= lerp(min_y_push, max_y_push, get_parent().get_position().y/obj.get_position().y)
