...
export var min_x_pull = 5
export var max_x_pull = 8
export var min_y_pull = 5
export var max_y_pull = 22
export var multi = 3
...
func _process(delta):
	...
	#my own 'custom' ways for scaling velocity based on how close object is to vacuum... lmk if u think of something better
			if(obj.get_position().x > get_parent().get_position().x):
				velocity.x -= lerp(min_x_pull, max_x_pull, get_parent().get_position().x/obj.get_position().x)
			else:
				velocity.x += lerp(min_x_pull, max_x_pull, get_parent().get_position().x/obj.get_position().x)
			if(obj.get_position().y > get_parent().get_position().y):
				velocity.y -= lerp(min_y_pull, max_y_pull, get_parent().get_position().y/obj.get_position().y)
			else:
				velocity.y += lerp(min_y_pull, max_y_pull, get_parent().get_position().y/obj.get_position().y)
